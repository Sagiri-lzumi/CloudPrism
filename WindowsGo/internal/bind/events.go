...
// ContextHolder 保存前台 context（对话框 API 曾依赖前台句柄；目录选择已改为
// 网页版选择器（web 层 /api/fs/* + LocalFS 域），holder 仅作装配兼容保留）。
//
// 绑定 struct 不能导出 SetCtx 之类的方法 —— 导出方法全会进入 JS 绑定面，
// 而 context.Context 是 Go 内部对象，前端根本无法构造。故需要前台 context
// 的域 struct 各自持有 *ContextHolder 指针（LocalFS 无状态、不持）。
// 本类型不进 Bind 数组（非绑定件）。
