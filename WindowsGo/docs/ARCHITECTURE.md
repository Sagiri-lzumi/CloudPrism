...
| 18 | 检查更新 UI | 设置页「关于」组提供「检查更新」按钮（对比 GitHub Release） | `pkg/update` checker 零生产引用，整包已于 v1.1 移除；「关于」组只展示 App.Version() 运行时诊断串，**v1.8 起另加一行前端自报的「界面构建」指纹**（`import.meta.url` 取本 bundle 名，用于分辨当前浏览器连的是哪个发布包 —— 单实例探测会把新 exe 静默顶掉） | Go 版暂无产品版本号载体；后续如需更新检查应基于发布包元数据重新设计 |
