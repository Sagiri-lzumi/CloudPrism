...
>
> ⚠️ **本表不可用于审计当前样式。** 表中「Go 现状」列记录的是 v1 时点的取值，
> v38 的视觉现代化改版已把其中多项推翻（圆角 8/4 → 卡片 12/控件 6、阴影由单层
> 改三层叠加、新增 `--bg-ambient` 环境光与 `--accent-grad` 品牌渐变、滚动条
> 10px → 12px、深色表面由 #202020/#292929 → #1a1a1e/#232327 等）。
> 当前真源只有 `theme.css`，本表保留的目的是记录「当时为什么这么对齐」。

> 本表**不再是活文档**：v38 起参考实现不存在，「逐条对照」这件事本身已经没有
> 对象。新视觉改动请直接改 `theme.css` 并在提交信息里说明动机，不要再往本表补行。
> 表内每条保留原始锚点，仅供回溯「当时对齐的是哪一处观感」。
